...
define_parsE <- function(input_df){
  ...
  par_highE =list(A=max(input_df$gs)*2,B=10,sd=100)
...
define_parsL <- function(input_df){
  ...
  par_highL=list(A=max(input_df$gs)*2, B=70, Xo=10, sd=100)
...
define_parsS <- function(input_df){
  ...
  par_highS=list(A=max(input_df$gs)*1.5, B=0, Xo=6, sd=100)
...
do_the_thing_linear <- function(input_df, model_type) {
  ...
  par_hi1= list(A=summary(fita)$coeff[1,1]*4, B =summary(fita)$coeff[2,1]*0.1, sd = 100)
